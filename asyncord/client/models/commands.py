...
class CreateApplicationCommandData(BaseModel):
    ...
    def validate_options(
        cls,
        field_value: list[ApplicationCommandOption] | None,
        ...
        field_value.sort(key=lambda item: item.required, reverse=True)
        return field_value
...
class ApplicationCommandOptionChoice(BaseModel):
    ...
    def validate_value(cls, field_value: str | int | float) -> str | int | float:
        ...
        if isinstance(field_value, str):
            if not (1 >= len(field_value) <= 100):
                raise ValueError('Value field must be 1-100 characters long')
        return field_value


CreateApplicationCommandData.model_rebuild()
ApplicationCommand.model_rebuild()
ApplicationCommandOption.model_rebuild()
